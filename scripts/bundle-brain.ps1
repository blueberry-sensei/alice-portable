...
#
#   - `sag_api` (105 file .py) + `sag_agent` + `alicecore` (87 file .py) là THUẦN
#     PYTHON (đã đếm: 0 .so, 0 .pyd) → copy thẳng từ image.
#   - Phần native cài lại từ PyPI bằng wheel Windows.
#   - Python là bản **embeddable** của python.org: giải nén là chạy, không cần cài,
...
# ── 3. Nguồn thuần Python từ image ─────────────────────────────────────────
Write-Output "[3/5] Copy sag_api + sag_agent + alicecore từ image…"
# `docker cp` chứ KHÔNG phải `docker exec tar | Set-Content`: PowerShell 5.1 chuyển
# stdout của native command thành CHUỖI, nên mọi byte nhị phân đi qua pipe đều hỏng
...
foreach ($item in @(
    ...
    @{ src = "$sitePkgs/alicecore-0.1.0.dist-info"; dst = "alicecore-0.1.0.dist-info" }
  )) {
...
# `__pycache__` là bytecode biên dịch cho CPython 3.12 trên LINUX. Trên Windows nó
# vô dụng và chỉ tổ phình bundle; Python sẽ tự sinh lại bản của nó.
...
# ── 4. Dependency native, wheel Windows từ PyPI ────────────────────────────
