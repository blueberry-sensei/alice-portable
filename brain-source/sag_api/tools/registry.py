"""Tool registry - look a tool up by name; new tools register here (mirrors connectors/registry)."""
...
class ToolRegistry:
    ...
    def schemas(self, names: list[str]) -> list[dict]:
        """Given a list of tool names -> a list of OpenAI function schemas (to pass as tools=)."""
    ...
    def overlay(self, tools: list[Tool]) -> ToolRegistry:
        """Derive an overlay: built-in tools + this request's MCP tools (the global singleton is untouched)."""
        child = ToolRegistry()
...
# Remote MCP tools are injected dynamically at runtime from the Agent bindings.
